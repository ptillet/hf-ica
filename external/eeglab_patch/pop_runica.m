...
% -------------------------
allalgs   = { 'runica' 'binica' 'jader' 'jadeop' 'jade_td_p' 'MatlabshibbsR' 'fastica' 'dshf_ica'...
...
for index = length(allalgs):-1:1
    if exist(allalgs{index}) ~= 2 & exist(allalgs{index}) ~= 6 & exist(allalgs{index}) ~= 3
        allalgs(index) = [];
    end;
end;
...
switch lower(g.icatype)
    ...
    case 'amica' 
     ...
     case 'dshf_ica',	   [EEG.icaweights,EEG.icasphere] = dshf_ica(tmpdata, struct(g.options{:}));
